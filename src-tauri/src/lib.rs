...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Tray
            build_tray(&app_handle).ok();

            // Position the main window at bottom-right and hide
            ...
            // mDNS discovery (sync, owns its threads internally)
            let st_disc = state.clone();
            match discovery::start(st_disc) {
                Ok(daemon) => {
                    app.manage(MdnsHolder(daemon));
                }
                Err(e) => tracing::error!(?e, "mdns failed to start"),
            }
        ...
        .expect("error while running localbeam");
}

struct MdnsHolder(mdns_sd::ServiceDaemon);
...
fn republish_discovery(
    ...
    daemon: tauri::State<'_, MdnsHolder>,
) -> Result<(), String> {
    let st = state.inner().clone();
    discovery::republish(&daemon.0, &st).map_err(|e| e.to_string())
}
